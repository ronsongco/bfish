...
Production-shaped adapter evaluation starts with [`translation-adapter-v2.jsonl`](Fixtures/translation-adapter-v2.jsonl) and [`ollama-translation-v2.txt`](Prompts/ollama-translation-v2.txt). Version 2 adds structured output, bounded context, omitted-subject cases with required and forbidden subject evidence, ASR-degraded text, uncertainty, negation, pipeline-suppression cases, and an adapter-direct non-speech defense case. Version 1 remains unchanged so its measurements stay reproducible; its free-form results must not be compared directly with version 2.
